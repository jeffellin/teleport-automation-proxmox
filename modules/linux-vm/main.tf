...
resource "proxmox_virtual_environment_vm" "linux_vm" {
  ...
  initialization {
    ...
    user_data_file_id = var.use_custom_user_data ? var.custom_user_data_file_id : proxmox_virtual_environment_file.cloud_init[0].id
  }
...
resource "proxmox_virtual_environment_file" "cloud_init" {
  count = var.use_custom_user_data ? 0 : 1
